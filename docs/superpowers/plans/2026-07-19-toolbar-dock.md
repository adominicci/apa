...
```css
/*
 * Por debajo de 1100px la barra vuelve abajo. La preferencia guardada NO se
 * toca: al agrandar la ventana la barra regresa sola a su lado.
 *
 * Por qué 1100 y no ~1400 (el ancho "ideal": 248 + 312 de paneles más 816px de
 * papel a 8.5in más la barra): el papel es `width: min(100%, 8.5in)`, así que
 * ya se achica solo — nunca exige sus 816px. El límite real es la ventana de
 * la app, que arranca en 1200px (tauri.conf.json); un breakpoint por encima de
 * eso dejaba la feature muerta al tamaño por defecto.
 */
