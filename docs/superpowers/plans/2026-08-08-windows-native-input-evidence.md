...
**Interfaces:** Produces `NativeManualInputMessage`, `parseNativeManualInputMessage(value)`, and `NativeProofBridge.postNativeInput(value): boolean`. The ordered stages are `ready`, `drag`, `copy`, `paste`, `caret`, `dead-keyup`, `dead-key`, and `undo`; the caret acknowledgement separates the real ArrowRight from the dead key, and the trusted dead-key release acknowledgement separates that key from the authored character.
...
**Interfaces:** Consumes the eight ordered `native-input` stages and produces driver actions plus final provenance metrics. A result is accepted for Windows manual mode only after the exact acknowledged caret advance, trusted dead-key release, composed-character outcome, and one-step undo restoration complete.
...
- Type consistency: browser messages and native states share the same eight-stage order.
