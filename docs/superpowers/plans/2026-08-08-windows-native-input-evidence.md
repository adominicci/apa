...
**Interfaces:** Consumes the eight ordered `native-input` stages and produces driver actions plus final provenance metrics. A result is accepted for Windows manual mode only after the exact acknowledged caret advance, trusted `Dead`/`Quote` keydown, composed-character outcome, and one-step undo restoration complete.
