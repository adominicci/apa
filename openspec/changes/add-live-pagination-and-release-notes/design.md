...
Before production integration, build a narrow browser/native proof that demonstrates: one paragraph splitting at a measured line boundary; caret and selection continuity across the visual gap; undo without pagination entries; a page start between table rows; an atomic figure moving as one unit; deterministic removal of a trailing page; and unchanged `editor.getJSON()`. Implementation SHALL stop and revise this design if those behaviors cannot be achieved reliably in WKWebView. A block-only paginator or fake page background is not an acceptable fallback for the approved requirements.
...
Pure planner tests use synthetic fragment geometry and cover exact break decisions without jsdom layout. ProseMirror tests cover decoration mapping and document immutability. Svelte/jsdom tests cover version resolution, canonical note-source precedence, Markdown safety, both launch controls, focus containment/return, page numbers, and status count wiring. A pinned test-only Playwright harness (Apache-2.0, after dependency audit) covers real layout in Chromium and WebKit: line rectangles, generated labels, tables, figures, lists, deletion reflow, scale invariance, and preview parity. A native Tauri/WKWebView run provides macOS evidence for editing across page boundaries and the two version entry points; the existing installer-build matrix remains a required cross-platform packaging gate.
...
- **DOM measurement differs between jsdom and WKWebView** → Keep the solver pure, use browser/native fixtures for geometry, and make the proof gate mandatory before full integration.
...
- **Visual transform may affect caret coordinates in older WebKit** → Test first paint, typing, selection, drag selection, and scroll-to-caret in the minimum supported macOS runtime before release.
...
4. Run full automated, browser, and native WKWebView evidence; do not release if the pagination proof or data-identity checks fail.
