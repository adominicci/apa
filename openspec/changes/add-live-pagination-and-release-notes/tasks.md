...
- [ ] 3.1 Bootstrap the two test-only native hosts before cross-engine RED tests: preserve the Swift WKWebView harness on macOS and add a Rust harness reusing Tesina's locked Tauri/Wry WebView2 backend on Windows; audit any new direct harness-only crate or SDK input under the existing license/security policy; serve the external-module proof from a readiness-checked loopback origin in a visible native window; require an external-module/visibility/animation-frame self-test; preflight and record the system Evergreen WebView2 runtime identity and fail closed when unavailable without promising a pinned Chromium version; add no Playwright package or separately downloaded browser bundle; and exclude all harness outputs and test tooling from shipped Tauri artifacts.
